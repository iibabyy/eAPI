...
impl ErrorMessage {
    fn to_str(&self) -> String {
        match self {
            ...
            ErrorMessage::RefreshTokenNotProvided => {
                "Refresh token not found, please log in".to_string()
            }
            ...
            ErrorMessage::NotEnoughProducts(stock) if stock > &0 => {
                format!("Only {stock} products remaining")
            }
...
impl From<BcryptError> for ErrorMessage {
    fn from(_value: BcryptError) -> Self {
...
impl<T> From<HttpError> for Result<T, HttpError> {
