...
pub async fn init_test_users(pool: &Pool<Postgres>) -> (Uuid, Uuid, Uuid) {
    ...
    (users_id[0], users_id[1], users_id[2])
}

pub async fn assert_user_infos(
    id: impl ToString,
    name: impl ToString,
    email: impl ToString,
    db_client: &DBClient,
...
pub async fn init_test_products(
    pool: &Pool<Postgres>,
) -> (TestProductData, TestProductData, TestProductData) {
    let db_client = DBClient::new(pool.clone());
    ...
    (products_id[0], products_id[1], products_id[2])
}
...
pub async fn init_test_orders(
    pool: &Pool<Postgres>,
) -> (TestOrderData, TestOrderData, TestOrderData) {
    let db_client = DBClient::new(pool.clone());
    ...
    (orders_data[0], orders_data[1], orders_data[2])
}
